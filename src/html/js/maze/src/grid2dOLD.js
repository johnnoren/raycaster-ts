export class Grid2d {
    ...
    isPartOfPerimeter([x, y]) {
        return y === this.topmostRow || y === this.bottommostRow || x === this.leftmostCol || x === this.rightmostCol;
    }
    forEach(callback) {
        this.grid.forEach((col, x) => { col.forEach((value, y) => { callback(value, x, y); }); });
    }
    isPerimeterIntersectionCell([x, y]) {
        if (!this.isPartOfPerimeter([x, y])) {
            return false;
        }
        if (x === this.leftmostCol || x === this.rightmostCol) {
            return this.isOdd(y);
        }
        return this.isOdd(x);
    }
    isInnerRegularCell([x, y]) {
        if (this.isPartOfPerimeter([x, y])) {
            return false;
        }
        if (this.isOdd(y)) {
    ...
    isAnyTypeOfInnerCell([x, y]) {
        return this.isInnerRegularCell([x, y]) || this.isInnerIntersectionCell([x, y]);
    }
    ...
    getInnerCellPositions() {
        return this.getPositions(this.isInnerRegularCell.bind(this));
    }
    ...
    getInnerCellPositionsByValue(value) {
        return this.getCellsByValue(value).filter(this.isInnerRegularCell.bind(this));
    }
